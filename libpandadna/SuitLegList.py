...
class SuitLegList:
    def __init__(self, path, dnaStore, suitWalkSpeed, fromSky, toSky, fromSuitBuilding, toSuitBuilding, toToonBuilding):
        ...
        zoneId = self.dnaStore.getSuitEdgeZone(startPoint.getIndex(), headingPoint.getIndex())
        startLeg = SuitLeg(self.getStartTime(0), zoneId, -1, startPoint, headingPoint, self.getFirstLegType())
        ...
        for i in xrange(1, self.path.getNumPoints()):
            ...
            leg = SuitLeg(self.getStartTime(i), zoneId, landmarkBuildingIndex, pointA, pointB, self.getNextLegType(i - 1))
        ...
        endLeg = SuitLeg(self.getStartTime(endIndex), zoneId, -1, endPoint, endPoint, self.getLastLegType())
